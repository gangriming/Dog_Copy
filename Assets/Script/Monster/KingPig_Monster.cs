...
public class KingPig_Monster : Monster
{
    ...
    private void Start()
    {
        ...
        monsterName = MonsterName.KING_PIG;
    ...
    private void FixedUpdate()
    {
        ...
        switch (monsterState)
        {
            ...
            case MonsterState.ATT:
                {
                    if (attTime <= 0f
                         && GetComponent<SpriteRenderer>().sprite.name == "Attack (38x28)_2")      // 특정 프레임에 공격 다른 방법이 있을까?
                    {
                        attTime = 0.6f;
                        ...
                        foreach (Collider2D item in collider2Ds)
                        {
                            if (tag == "SummonMonster" && item.gameObject.tag == "EnemyMonster")
                            {
                                ...
                                item.gameObject.GetComponent<Monster>().SetHp(15);
